...
3. Applies both tables under a single store write-lock (`index::apply_records` takes the records by value — moved into the store, no clone — plus watermark write-back) and emits `message.new` / `message.revoke` events on a tokio broadcast channel (capacity 1024); recall messages are detected by the parser (`MsgType::Recall`). Response rows are shaped in the same lock (`MessageOut` with the mediaId fetchability filter, shared with the messages query).
